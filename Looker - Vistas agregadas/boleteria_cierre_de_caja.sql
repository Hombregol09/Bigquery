-- ============================================================================
-- Vista: boleteria_cierre_de_caja
-- Dataset: firestore_export
-- Proyecto: casla-fanhub-admin-prod
-- Descripción: Vista agregada para cierre de caja en boletería.
-- ============================================================================
...
WITH                                                                                              
    ...
    -- Nuevos CTEs para eliminar duplicados en las tablas de JOIN
    UniqueUsers AS (
    ...
    UniquePlaces AS (
      SELECT sector_id, MAX(sector_name) AS sector_name, MAX(sector_code) AS sector_code
    ...
    UniqueMembers AS (
      SELECT id, MAX(name) AS name, MAX(last_name) AS last_name, MAX(phone) AS phone, MAX(email) AS email, MAX(dni) AS dni, MAX(member_number) AS member_number
    ...
    UniqueEvents AS (
      SELECT
        event_id,
        MAX(description) AS event_description
  ...
  LEFT JOIN UniqueUsers AS u                                                                        
    ON tp.admin_user_id = u.id                                                                      
  LEFT JOIN UniquePlaces AS pl                                                                      
    ON tp.tickets_package_sector_id = pl.sector_id                                                  
  LEFT JOIN UniquePayments AS p                                                                     
    ON tp.Payments_Relation_ID = p.id                                                               
  LEFT JOIN UniqueMembers AS m                                                                      
    ON tp.Members_Relation_ID = m.id                                                                
  LEFT JOIN UniqueEvents AS e                                                                       
